...
void *mymalloc(uint32_t size)
{
	...
	if (offset == 0XFFFFFFFF)
		...
		return (void *)((uint32_t)mallco_dev.membase[SRAMIN] + offset);
}
...
void *myrealloc( void *ptr, uint32_t size)
{
	...
	{
		mymemcpy((void *)((uint32_t)mallco_dev.membase[SRAMIN] + offset), ptr, size); //拷贝旧内存内容到新内存
		myfree(ptr);																//释放旧内存
		return (void *)((uint32_t)mallco_dev.membase[SRAMIN] + offset);				//返回新内存首地址
	}
}
